...
if (OnDoubleClick or OnChar or OnSwitchFile) and not extman.Command then
  Error("There is a handler conflict, please use extman")
else
  -- simple way to add a handler only, can't remove like extman does
  if not extman.OnDoubleClick then
    local _OnDC
    extman.OnDoubleClick = function(f) _OnDC = f end
    OnDoubleClick = function(c) if _OnDC then return _OnDC(c) end end
  end
  if not extman.OnChar then
    local _OnCh
    extman.OnChar = function(f) _OnCh = f end
    OnChar = function(c) if _OnCh then return _OnCh(c) end end
  end
  if not extman.OnSwitchFile then
    local _OnSF
    extman.OnSwitchFile = function(f) _OnSF = f end
...
local function HexEditorBase(NoCurrent)
  ...
  --------------------------------------------------------------------
  extman.OnDoubleClick(HandleClick)      -- set up handlers
  extman.OnChar(HandleChar)
  extman.OnSwitchFile(HandleSwitchFile)
