...
-- luacheck: globals Dbg

local GTK = extman.GetProp('PLAT_GTK')

local function do_set_menu()
  extman.Command {
...
local dbg
local catdbg
local closing_process

function dbg_last_command()
    return dbg.last_command
end

local function dbg_status()
    return status
end

local function dbg_obj()
    return dbg
end
...
-- be strings.
-- local pretty_print_limit = tonumber(extman.GetProp('pretty.print.limit',20))
...
-- Lua statement)
extman.OnOutputLine (function (line)
...
local debug_status = extman.GetProp('debug.status',false)
...
local function breakpoint_from_position(lno)
    for b in Breakpoints() do
        if b.file == extman.CurrentFile() and b.line == lno then
            return b
        end
    end
    return nil
...
function do_backtrace()
    dbg:backtrace(extman.GetProp('debug.backtrace.depth','20'))
end
...
local function create_existing_breakpoints()
...
local function create_debugger(ext,target)
...
function do_launch()
    ...
    local target = extman.GetProp('debug.target')
    local ext
    if target then
    ...
    root = props['FileDir']
    dbg:init(root)
    do_skip_includes = extman.GetProp('debug.skip.includes',true)
    if do_skip_includes then
        ...
        skip_file_pattern = extman.GetProp(file_pat_prop,inc_path)
    end
    ...
    target = extman.quote_if_needed(target)
    ...
    if extman.GetProp('debug.output') then
        catdbg = io.open('/tmp/scite-debug-out','w')
    else
        catdbg = nil
    end
    --- and go!!
    spawner.verbose(extman.GetPropBool('debug.spawner.verbose',false))
--  spawner.fulllines(1)
...
local function dbg_command_line(s)
...
--  print('*',line)
    ...
    local prog_ended,process_finished = dbg:detect_program_end(line)
    if prog_ended then
        if not process_finished then spawner_command('quit') end
        set_status('dead')
        closing_process()
        return
    end
...
local function calltip(s)
    editor:CallTipShow(_pos,s)
end

extman.OnDwellStart(function (pos,s)
    if status == 'active' or status == 'error' then
        if s ~= '' then
            s = current_expr(pos)
            _pos = pos
