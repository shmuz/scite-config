...
-- luacheck: new_globals smart_paste smart_copy block_select block_end
extman.Command("Smart Paste|smart_paste|Shift+Ctrl+V")
extman.Command("Smart Copy|smart_copy|Shift+Ctrl+C")
extman.Command("Block Select|block_select|Shift+Ctrl+A")
extman.Command("Match Block End|block_end|Ctrl+E")

local function indentation_of_line (line)
...
local function goto_line_start(line)
    local pos = editor:PositionFromLine(line)
    editor:GotoPos(pos)
    return pos
end

local function goto_line_end(line)
...
--~ 		print(line,new_indent,editor:GetLine(line))
    ...
    if indent ~= -1 then
        ...
        if diff > 0 then
            for _ = 0,diff-1 do editor:Tab() end
        elseif diff < 0 then
            diff = -diff
            for _ = 0,diff-1 do editor:BackTab() end
        end
    end
