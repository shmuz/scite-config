...
-- luacheck: read_globals SCFIND_REGEXP SC_MARK_BACKGROUND SC_MARK_ARROW
-- luacheck: read_globals INDIC0_MASK INDIC1_MASK INDIC2_MASK

extman.require 'class.lua'
...
function path_of (s)
    local _,idx = split_last(s,dirsep)
    if idx then
...
function Marker:delete()
    if self.file ~= extman.CurrentFile() then -- not the correct buffer!
        self.state = 'expired'
    else
...
MarkerType = class(function(self,idx,typ,fore,back)
    ...
    extman.OnSwitchFile(function(f)
...
function MarkerType:for_file(fname)
    if not fname then fname = extman.CurrentFile() end
    local i = 0
    local n = #self.markers -- table.getn(self.markers)
